...
needs to call the currency tool based on the trip request, we execute
whatever it asks for, and feed results back until it's done.

Weather is gathered separately in llm_service from the resolved destination
(geocode + daily forecast), so this loop is not responsible for it.
...
agent step runs once, up front, to gather optional context (e.g. a stated
budget converted to local currency) that then gets folded into the itinerary
prompts as plain text -- the itinerary writer never has to make
...
a currency conversion tool. Given a trip request, call it only if the user \
mentioned a budget in a specific currency and converting it would help. \
Don't call tools for things you already know or that don't need real-time \
data. Weather is gathered separately -- do not try to look it up. Once you \
have what you need (or if no tools are needed), reply with a short \
plain-text summary, 2-4 sentences, of anything useful you found that \
