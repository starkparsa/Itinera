...
# source of truth for client construction/model name/thinking-config now
# (shared with agent_service.py's tool-calling loops, which used to reach
# into this module's underscore-prefixed internals for the same three
# things). Kept as local names here, not inlined at each call site below,
# purely so the existing test suite's `patch("app.llm_service._get_client",
# ...)` call sites keep working unchanged -- see gemini_client.py's
# docstring for the full rationale.
GEMINI_API_KEY = gemini_client.GEMINI_API_KEY
GEMINI_MODEL = gemini_client.GEMINI_MODEL
_THINKING_CONFIG = gemini_client.THINKING_CONFIG
_get_client = gemini_client.get_client
