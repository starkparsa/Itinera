...
backend/app/
  ...
  agent_service.py        TWO tool-calling loops: gather_trip_context (currency, paused, kill switch off) and answer_question_with_tools (place-context, on by default, runs fresh every question turn) -- see decision log's Place context row for why they're kept separate
  tools.py                 Tool implementations + schemas (currency via Frankfurter, weather removed; get_place_context via clients/wikipedia_client.py) -- per-loop schema subsets (CURRENCY_TOOL_SCHEMAS/QA_TOOL_SCHEMAS), see decision log
