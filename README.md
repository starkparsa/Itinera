...
**Option A — Docker Compose (recommended, one command, includes MySQL):**
```bash
docker compose up --build
```
- Frontend: http://localhost:3000
- Backend API docs: http://localhost:8000/docs
- MySQL: `localhost:3307` (user `travel_user` / pass `travel_pass`) — mapped
  to 3307, not the default 3306, in case you already have a local MySQL
...
```
This points at MySQL by default (`DATABASE_URL` in `.env`) — either point
it at a MySQL instance you're running some other way, start just that one
service (`docker compose up mysql`, still mapped to host port 3307, so
update `DATABASE_URL` accordingly), or skip MySQL entirely for a quick run
by overriding to a local SQLite file instead (confirmed working — this is
how this project's own local dev instance runs day to day):
```bash
...
SQLite/MySQL instance work with zero setup. Alembic (`backend/alembic/`) is
